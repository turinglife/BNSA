...
###################################################################
# Description: Convert list to vector and uppercase to lowercase
# Author: Xin Zhu
# Date: 2014-01-21
# Comments: 
###################################################################
convert_l_to_v_with_lowercase <- function(list_obj)
{
	vector_obj <- vector()

	# generate vector object
	for(index in 1:length(unlist(list_obj)))
	{
		vector_obj[index] <- c(tolower(list_obj[[1]][index]))
	}
